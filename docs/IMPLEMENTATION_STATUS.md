...
| ADR-002 SQLite embedded profile store | `src-tauri/src/infrastructure/sqlite/*`, exact-pinned bundled SQLite via rusqlite | **partially implemented: profile/schema/migration adapter exists; XDG/instance lifecycle remains A06** | A06 |
| A05 SQLite schema + atomic migration engine | schema v1, min-reader/min-writer metadata, WAL/FULL/FK/busy policy, backup+journal+integrity migration engine, SQLite repository adapter + shared A04 contract/reopen/failure tests | **implemented at source/test level; first UTS compile exposed the backup API defect corrected by A05b; repeat Cargo verification required** | A06 |
| A05b rusqlite backup API compile correction | `migration.rs` uses rusqlite 0.40 `Name` API via `"main"`; `tools/check_a05b.py`; `docs/evidence/A05B_UTS_COMPILE_FIX.md` | **implemented; UTS Cargo verification required** | A06 |
