...
| A05 SQLite schema + atomic migration engine | schema v1, min-reader/min-writer metadata, WAL/FULL/FK/busy policy, backup+journal+integrity migration engine, SQLite repository adapter + shared A04 contract/reopen/failure tests | **implemented at source/test level; UTS Cargo verification required** | A06 |
